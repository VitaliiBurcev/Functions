...
void FillRand(char arr[], const int arroys_arroys_limit);
...
void main()
{
	...
	Print(arr, n);
	cout << "Введите количество сдвигов (сдвиг вправо): "; 
	...
	const int arroys_limit = 10;
	int krr[arroys_limit];
	FillRand(krr, arroys_limit);
	Print(krr, arroys_limit);
	Sort(krr, arroys_limit);
	Print(krr, arroys_limit);
	cout << "Сумма элементов массива: " << Sum( krr, arroys_limit) << endl;
	cout << "Средне-арифметическое элементов массива: " << Avg( krr, arroys_limit) << endl;
	cout << "Минимальное значение: " << minValueIn(krr, arroys_limit) << endl;
	cout << "Максимальное значение: " << maxValueIn(krr, arroys_limit) << endl;
	cout << "Введите количество сдвигов (сдвиг влево): "; cin >> number_of_shifts;
	shiftLeft(krr, arroys_limit, number_of_shifts);
	Print( krr,arroys_limit);
	cout << "Введите количество сдвигов (сдвиг вправо): "; cin >> number_of_shifts;
	shiftRight(krr, arroys_limit, number_of_shifts);
	Print(krr,arroys_limit);
	...
	Sort( arr[ROWS][COLS], ROWS, COLS);
	Print(arr[ROWS][COLS], ROWS, COLS);
}
...
void FillRand(char[], const int arroys_limit)
{
	// Заполнение массива случайными числами
	for (int i = 0; i < arroys_limit; i++)
	{
		krr[i] = rand() % 100;
	}
}
